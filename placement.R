optimization <- function(estimation, fleet_size, seed){
  set.seed(seed)
...
#initial placement
    ...
    initial_placement <- data.frame(start_station = all_original_stations) %>%
      left_join(placement_plan, by = "start_station") %>%
      mutate(placement_bikes = tidyr::replace_na(placement_bikes, 0))
  
#organizing inventory
  current_inventory <- setNames(initial_placement$placement_bikes, 
                                initial_placement$start_station)
...
print(optimization(complete_estimated_arrivals, , 123))
